...
class Job(ABC):
    ...
    def default_conf(self):
        raise NotImplementedError("Please implement default_conf method")

    def _prepare_conf(self, conf):
        if conf is not None:
            self.logger.info("Using provided conf object")
            self.conf = conf
        else:
    ...
    def _provide_config(self):
        ...
        if not conf_file:
            self.logger.info("No conf file was provided, using default configuration setting")
