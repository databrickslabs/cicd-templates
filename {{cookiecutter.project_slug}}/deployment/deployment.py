        ...
        for handle in submitted_jobs:
            ...
            if res is not None:
                if res == 'SUCCESS':
                    ...
                    print('Job ' + handle['run_id'] + ' was not successful.')
                    return False
...
def update_job_spec_for_prod_jobs(client, root_folder, artifact_uri, libraries, cloud):
    for file in listdir(root_folder):
        pipeline_path = join(root_folder, file)
        if isdir(pipeline_path):
            job_spec = check_if_dir_is_pipeline_def(pipeline_path, cloud)
            if job_spec is not None:
                adjust_job_spec(job_spec["new_settings"], artifact_uri + '/job/' + pipeline_path, libraries)
                print(job_spec)
